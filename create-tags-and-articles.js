...
const TAGS_TO_CREATE = [
  'paris', 'bangkok', 'air-france', 'vol-direct', 'thailande',
  'japon', 'visa', 'simplification', 'voyage', 'actualite',
  'hotel', 'bon-plan', 'promo', 'guide', 'pratique', 'prix', 'budget'
];

// Articles FlashVoyages à créer (sans tags pour l'instant)
...
async function createTags() {
  console.log('🏷️ Création des tags FlashVoyages...');
  
  const createdTags = {};
  
  for (const tagName of TAGS_TO_CREATE) {
    try {
      const response = await axios.post(`${WORDPRESS_URL}/wp-json/wp/v2/tags`, {
        name: tagName,
      ...
      createdTags[tagName] = response.data.id;
      console.log(`✅ Tag créé: ${tagName} (ID: ${response.data.id})`);
      
    } catch (error) {
      if (error.response?.status === 400 && error.response?.data?.code === 'term_exists') {
        // Le tag existe déjà, on récupère son ID
        const existingTags = await axios.get(`${WORDPRESS_URL}/wp-json/wp/v2/tags?search=${tagName}`, {
          auth: {
            username: WORDPRESS_USERNAME,
            password: WORDPRESS_APP_PASSWORD
          }
        ...
        if (existingTags.data.length > 0) {
          createdTags[tagName] = existingTags.data[0].id;
          console.log(`ℹ️ Tag existant: ${tagName} (ID: ${existingTags.data[0].id})`);
        }
      } else {
        console.error(`❌ Erreur tag ${tagName}:`, error.response?.data?.message || error.message);
      }
    }
  }
...
async function createFlashVoyagesArticles(tags) {
...
async function main() {
  try {
    ...
    // Pause avant de créer les articles
    await new Promise(resolve => setTimeout(resolve, 2000));
    
    // Créer les articles
    await createFlashVoyagesArticles(tags);
