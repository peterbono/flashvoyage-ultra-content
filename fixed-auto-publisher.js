...
async function getOrCreateTag(tagName) {
  try {
    ...
    const searchResponse = await axios.get(`${WORDPRESS_URL}/wp-json/wp/v2/tags?search=${encodeURIComponent(tagName)}`, {
      auth: {
        username: WORDPRESS_USERNAME,
        password: WORDPRESS_APP_PASSWORD
      }
...
async function publishUltraContent(content) {
  ...
  try {
    ...
    // Créer les tags
    let tagIds = [];
    const tagNames = [content.type, content.destination || 'asie', 'ultra-specialise'];
    ...
    console.error(`   ❌ Erreur publication: ${error.response?.data?.message || error.message}`);
    return null;
  }
}
