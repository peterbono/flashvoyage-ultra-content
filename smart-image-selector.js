...
// Base d'images Pexels thématiques et contextuelles
...
function selectContextualImage(destination, themes) {
  const destinationImages = THEMATIC_IMAGES[destination] || THEMATIC_IMAGES['thailande'];
...
function generateContextualAltText(title, destination, themes) {
  const themeNames = {
    'vols': 'vols',
    'hotels': 'hôtels',
    'visa': 'visa',
  ...
  return `${title} - Image ${primaryTheme} ${destinationName}`;
}

async function updateArticlesWithContextualImages() {
  console.log('🎯 Mise à jour des articles avec images contextuelles...');
  ...
  try {
    ...
    for (const article of articles) {
      ...
      // Sélectionner l'image contextuelle
      const imageUrl = selectContextualImage(destination, themes);
      console.log(`   Image sélectionnée: ${imageUrl}`);
      
      // Uploader la nouvelle image
      const filename = `${destination}-${themes[0] || 'default'}-${article.id}-${Date.now()}.jpg`;
      const altText = generateContextualAltText(article.title.rendered, destination, themes);
      ...
      try {
        ...
        console.log(`   ✅ Image contextuelle ajoutée (ID: ${uploadResponse.data.id})`);
    ...
    console.log('✅ Alt text générés automatiquement');
    console.log('✅ Cohérence contenu/image améliorée');
...
updateArticlesWithContextualImages();
